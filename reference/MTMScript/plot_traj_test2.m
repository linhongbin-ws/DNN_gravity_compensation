...
[SLSE1OL_dynamic_vec, ~] = slse(train_data_Collection_file);
SLSE1POL_tor_mat = compute_tor_mat('SLSE1POL', pos_mat, vel_mat,SLSE1OL_dynamic_vec);
...
for i = 1:7
    ...
    plot(x, SLSE1POL_tor_mat(i,:),'g','LineWidth',model_line_width);
    plot(x, MLSE_tor_mat(i,:),'r','LineWidth',our_model_line_width);
    hold off
    if i~=7
        set(gca,'XTick',[]);
    end
    if i ==1
         lgd = legend({'measured', 'CAD', 'Fontanelli et al.' ,'Our method'} ,'NumColumns',4,'Location','northoutside')
         set(lgd,'FontName','Times New Roman');
set(lgd,'FontName','Times New Roman');
    end
...
SLSE1POL_RMS_err_mat = SLSE1POL_tor_mat(:,choose_index_list)-tor_mat(:,choose_index_list);
SLSE1POL_RMS_vec = sqrt(sum(SLSE1POL_RMS_err_mat.^2,2)/size(SLSE1POL_RMS_err_mat,2))
...
function tor_mat = compute_tor_mat(method, pos_mat, vel_mat, dynamic_vec)
    ...
    for i = 1:size(pos_mat,2)
        ...
        elseif  strcmp(method,'MLSE') || strcmp(method,'SLSE1POL')
