function [output_dynamic_matrix, RMS_Torques_data, Condition_num_arr] = mlse(dataCollection_info_str, data_shrink_rate)
    ...
    for i=6:-1:1
        if i==6
            [~, Torques_pos_data, Torques_neg_data, Condition_num_arr(i)] = lse_mtm_one_joint(config_lse_list{i},data_shrink_rate);
            Torques_data_mat = cat(3, Torques_pos_data, Torques_data_mat);
            Torques_data_mat = cat(3, Torques_neg_data, Torques_data_mat);
        elseif i==1
            [output_dynamic_matrix, Torques_pos_data, Torques_neg_data, Condition_num_arr(i)] = lse_mtm_one_joint(config_lse_list{i},data_shrink_rate,config_lse_list{i+1});
            Torques_data_mat = cat(3, Torques_pos_data, Torques_data_mat);
            Torques_data_mat = cat(3, Torques_neg_data, Torques_data_mat);
        elseif i==5
            [output_dynamic_matrix, Torques_pos_data, Torques_neg_data, Condition_num_arr(i)] = lse_mtm_one_joint(config_lse_list{i},data_shrink_rate,config_lse_list{i+1});
            Torques_data_mat = cat(3, Torques_pos_data, Torques_data_mat);
            Torques_data_mat = cat(3, Torques_neg_data, Torques_data_mat);
            RMS_Torques_data = Torques_pos_data; 
        else
            [~, Torques_pos_data, Torques_neg_data, Condition_num_arr(i)] = lse_mtm_one_joint(config_lse_list{i},data_shrink_rate,config_lse_list{i+1});
            Torques_data_mat = cat(3, Torques_pos_data, Torques_data_mat);
            Torques_data_mat = cat(3, Torques_neg_data, Torques_data_mat);
        end
    end
...
%     % test data reading
%     fid = fopen(test_dataCollection_info_str);
%     if fid<3
%         error('Cannot read file %s', test_dataCollection_info_str)
%     end
%     raw = fread(fid, inf);
%     str = char(raw');
%     fclose(fid);
%     config_test = jsondecode(str);
%     config_test.lse = config.lse;
%     [test_input_data_path_with_date, ~, ~] = fileparts(test_dataCollection_info_str);
%     config_lse_list_test=setting_lse(config_test,test_input_data_path_with_date);
%     for i=5
%         lse_obj = lse_preparation(config_lse_list_test{i});
%         Torques_pos_data = lse_obj.Torques_pos_data;
%         Torques_neg_data = lse_obj.Torques_neg_data;
%     end
%     test_data_RMS = Torques_pos_data;
...
%     train_RMS = RMS_test(train_data_RMS, 5, output_dynamic_matrix, 'pos', config.lse.g_constant);
%     test_RMS = RMS_test(test_data_RMS, 5, output_dynamic_matrix, 'pos', config.lse.g_constant)
...
function [dynamic_parameters_vec, Torques_pos_data, Torques_neg_data, Condition_num] = lse_mtm_one_joint(config_lse_joint,data_shrink_rate, previous_config)
    ...
    if ~exist('previous_config')
        [dynamic_parameters_vec, Torques_pos_data, Torques_neg_data, Condition_num] = lse_model(config_lse_joint,data_shrink_rate);
    else
        % if there is 'previous_config', we pass the path to the result of previous step of LSE to lse_model
        [dynamic_parameters_vec, Torques_pos_data, Torques_neg_data, Condition_num] = lse_model(config_lse_joint,...
            data_shrink_rate,...
            previous_config.output_param_path);
    end
end

function [dynamic_parameters_vec, Torques_pos_data, Torques_neg_data,Condition_num] = lse_model(config_lse_joint1,...
                                            data_shrink_rate,...
                                            old_param_path)
    ...
    lse_obj = lse_preparation(config_lse_joint1,data_shrink_rate);
    ...
    [output_param_map,output_param_full_map, output_param_rel_std_map,Condition_num] = lse(input_param_map,...
...
function lse_obj = lse_preparation(config_lse_joint,data_shrink_rate)
    ...
    for i=1:size(data_path_struct_list,1)
        ...
        tmp_mat = lse_obj.Torques_pos_data_list{end};
        data_size = size(lse_obj.Torques_pos_data_list{end},3);
%         while(size(tmp_mat,3)>data_size*data_shrink_rate)
%             tmp_mat(:,:,randi([1 size(tmp_mat,3)])) = [];
%         end
        lse_obj.Torques_pos_data_list{end} = tmp_mat(:,:,1:round(data_size*data_shrink_rate));
    end
    ...
    for i=1:size(data_path_struct_list,1)
        ...
        tmp_mat = lse_obj.Torques_neg_data_list{end};
        data_size = size(lse_obj.Torques_neg_data_list{end},3);
%         while(size(tmp_mat,3)>data_size*data_shrink_rate)
%             tmp_mat(:,:,randi([1 size(tmp_mat,3)])) = [];
%         end
        lse_obj.Torques_neg_data_list{end} = tmp_mat(:,:,1:round(data_size*data_shrink_rate));
    end
...
% function RMS = RMS_test(Torques_data, Joint_No, param_vec, dir, g)
%     err_array =[];
%     for i = 1:size(Torques_data,3)
