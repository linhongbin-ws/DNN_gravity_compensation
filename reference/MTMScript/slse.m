function [dynamics_param_vec,Torques_data] = slse(test_dataCollection_info_str, data_shrink_rate)
    ...
    T2_augmented = [];
    for i=1:6
        lse_obj = lse_preparation(config_lse_list_test{i}, data_shrink_rate);
        Torques_data = cat(3,Torques_data,lse_obj.Torques_pos_data);
        Torques_data = cat(3,Torques_data,lse_obj.Torques_neg_data);
        R2_augmented = [R2_augmented; lse_obj.R2_augmented_pos; lse_obj.R2_augmented_neg];
        T2_augmented = [T2_augmented; lse_obj.T2_augmented_pos; lse_obj.T2_augmented_neg];
    end
    dynamics_param_vec = pinv(R2_augmented)*T2_augmented;
end




function [dynamic_parameters_vec, Torques_pos_data, Torques_neg_data] = lse_model(config_lse_joint1,...
                                            old_param_path)
...
%         end
        tmp_mat(:,:,1:round(data_size*data_shrink_rate));
        lse_obj.Torques_pos_data_list{end} = tmp_mat;
    end
...
%             tmp_mat(:,:,randi([1 size(tmp_mat,3)])) = [];
%         end
        tmp_mat = tmp_mat(:,:,1:round(data_size*data_shrink_rate));
        lse_obj.Torques_neg_data_list{end} = tmp_mat;
    end
...
function  [R2_augmented, T2_augmented] = data2augmat(Torques_data,...
    ...
    for i=1:size(Torques_data,3)
        if strcmp(direction, 'pos')
            if(nargin('analytical_regressor_pos_mat')==7)
                R = analytical_regressor_pos_mat(g,...
                                                 ...
                                                 Torques_data(5,1,i),...
                                                 Torques_data(6,1,i));
            elseif(nargin('analytical_regressor_pos_mat')==6)
                R = analytical_regressor_pos_mat(g,...
                                                 Torques_data(2,1,i),...
                                                 Torques_data(3,1,i),...
                                                 Torques_data(4,1,i),...
        ...
        elseif strcmp(direction, 'neg')
             if(nargin('analytical_regressor_neg_mat')==7)
                R = analytical_regressor_neg_mat(g,...
                                                 ...
                                                 Torques_data(5,1,i),...
                                                 Torques_data(6,1,i));
            elseif(nargin('analytical_regressor_neg_mat')==6)
                R = analytical_regressor_neg_mat(g,...
                                                 Torques_data(2,1,i),...
                                                 Torques_data(3,1,i),...
                                                 Torques_data(4,1,i),...
