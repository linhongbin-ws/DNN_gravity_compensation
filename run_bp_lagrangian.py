...
earlyStop_patience = 30 # epoch number of looking ahead
...
# train model
model = train_lagrangian(model, train_loader, valid_loader, optimizer, loss_fn, early_stopping, max_training_epoch, delta_q=1e-5)
# test model
test_loss, abs_rms_vec, rel_rms_vec = test_lagrangian(model, loss_fn, test_data_path, input_scaler, output_scaler, delta_q=1e-5, device=device)
