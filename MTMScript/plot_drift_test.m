load('./data/drift_test/drift_test_CAD_SLSE_MLSE4POL.mat');
addpath('./gen_code')
...
for i=1:size(CAD_pose_mat_cell_list,2)
    start_joint_pos_mat(:,i,2) = SLSE_WPI_pose_mat_cell_list{i}(:,1);
end
for i=1:size(CAD_pose_mat_cell_list,2)
...
for i=1:size(CAD_pose_mat_cell_list,2)
    end_joint_pos_mat(:,i,2) = SLSE_WPI_pose_mat_cell_list{i}(:,end);
end
for i=1:size(CAD_pose_mat_cell_list,2)
...
% ylim([0 50])
lgd = legend('CAD','SLSE','Our method')
...
% y = rad2deg(SLSE_WPI_pose_mat_cell_list{plot_data_set_index}(plot_joint,:));
% x = linspace(0,2,size(y,2));
...
% lgd = legend({'CAD','SLSE','Our method'},'NumColumns',3,'Location','north');
% set(lgd,'FontName','Times New Roman');
...
[transl_cell_list{end+1},ori_cell_list{end+1}]  = compute_catesian(SLSE_WPI_pose_mat_cell_list{plot_data_set_index});
