...
def loop_func(train_data_path, valid_data_path, test_data_path, use_net):
    ...
    K_trainLoader, K_validLoader, _, K_output_scaler = load_teacher_train_data(Mid_teacherModel, teacher_sample_num, batch_size, device, input_scaler=input_scaler, is_outputScale=True, train_ratio=0.8)

    ### Train K Model
    loss_fn = torch.nn.SmoothL1Loss()
    optimizer = torch.optim.Adam(model[0].parameters(), lr=learning_rate, weight_decay=weight_decay)
    early_stopping = EarlyStopping(patience=earlyStop_patience, verbose=False)
    model[0] = train(model[0], K_trainLoader, K_validLoader, optimizer, loss_fn, early_stopping, 50, goal_loss,
                  is_plot=False)
...
loop_func(train_data_path, valid_data_path, test_data_path, 'Two_ReLuNet')
